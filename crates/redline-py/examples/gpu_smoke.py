...
the counter back -- it must equal N iff every dispatch executed to completion.
    ...
    ROCR_VISIBLE_DEVICES=3 python gpu_smoke.py 256 counter.co counter.radiowave.json
"""
import sys
...
if manifest_path:
    ...
    manifest = None
mod = gpu.load_module(code, manifest)
...
ok = val == N
print(
    f"PyO3 real-GPU gate: counter = {val} / {N} "
    f"certified={mod.radiowave_certified} [{'PASS' if ok else 'FAIL'}]"
)
